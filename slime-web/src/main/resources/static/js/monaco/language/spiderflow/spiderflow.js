...
function SpiderflowGrammar(clazz){
    this.clazz = clazz;
    this.init();
}
SpiderflowGrammar.prototype.reset = function(clazz){
    this.clazz = clazz;
    this.init();
}
SpiderflowGrammar.prototype.init = function(){
    if(this.clazz){
...
SpiderflowGrammar.prototype.findHoverSuggestion = function(inputs){
    var target;
    var method;
    var owner;
...
SpiderflowGrammar.prototype.findSuggestions = function(inputs){
    var target;
    for(var i=0;i<inputs.length;i++){
        var input = inputs[i];
        if(!target){
...
var spiderflowGrammar = new SpiderflowGrammar();
require(['vs/editor/editor.main'], function() {
    ...
    monaco.languages.registerHoverProvider('spiderflow',{
        provideHover : function(model,position,token){
            ...
            if(tokens.length > 0 && tokens[tokens.length - 1].getTokenType() == TokenType.Identifier){
                ...
                return {
                    contents : spiderflowGrammar.findHoverSuggestion(array)
                }
            }
            return {
                contents : []
    ...
    monaco.languages.registerCompletionItemProvider('spiderflow',{
        provideCompletionItems : function(model,position,context,token){
            ...
            return {
                suggestions : spiderflowGrammar.findSuggestions(array)
            };
        },
        triggerCharacters : ['.','{']
    })
